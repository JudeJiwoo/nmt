...
class Corpus2event_remi():
  ...
  def _half_step_interval_gap_check(self, instrument_note_dict, marking_instrument):
      # Sort the notes by pitch for easier comparison
      sorted_notes = sorted(instrument_note_dict[marking_instrument].items())
      pitch_list = [pitch for pitch, _ in sorted_notes]
      ...
      # Iterate through the pitch list and compare adjacent notes for half-step interval gap
      for idx in range(len(pitch_list) - 1):
          current_pitch = pitch_list[idx]
          next_pitch = pitch_list[idx + 1]

          if next_pitch - current_pitch == 1:  # Check if there's a half-step interval gap
              current_max_duration = max(duration for duration, _ in instrument_note_dict[marking_instrument][current_pitch])
              next_max_duration = max(duration for duration, _ in instrument_note_dict[marking_instrument][next_pitch])

              if current_max_duration < next_max_duration:
                  new_pitch_list.append(next_pitch)  # Keep the higher pitch
              else:
                  new_pitch_list.append(current_pitch)  # Keep the lower pitch
          else:
              new_pitch_list.append(current_pitch)  # No half-step gap, so keep the current pitch

      # Ensure the last pitch is added if it's not part of a half-step interval
      if pitch_list[-1] not in new_pitch_list:
          new_pitch_list.append(pitch_list[-1])

      # Filter out notes not in new_pitch_list and create the new dictionary
      instrument_note_dict[marking_instrument] = {
          pitch: instrument_note_dict[marking_instrument][pitch]
          for pitch in new_pitch_list
      }
              ...
              for in_beat_off_idx, beat_step in enumerate(frange(measure_step, measure_step + bar_resol, in_beat_tick_resol)):
                  ...
                  for instrument_idx in instrument_list:
                      ...
                      if t_notes is not None:
                          # Add chord and tempo changes if they differ from the previous ones
                          ...
                          # Add instrument change event if necessary
                          ...
                          # Check and handle half-step interval gaps
                          self._half_step_interval_gap_check(instrument_note_dict, instrument_idx)

                          # For each pitch, select the note with the longest duration and create note events
                          for note_dict in instrument_note_dict[instrument_idx]:
                              max_duration = max(instrument_note_dict[instrument_idx][note_dict], key=lambda x: x[0])
                              note_event = [
                                  self._create_event('Note_Pitch', note_dict),
                                  self._create_event('Note_Duration', max_duration[0])
                              ]
                              if self.num_features in {7, 8}:
                                  note_event.append(self._create_event('Note_Velocity', max_duration[1]))  # Add velocity if applicable
      ...
      for idx in range(len(time_signature_changes)):
        ...
        for measure_step in frange(bar_start_tick, next_change_point, bar_resol):
          ...
          for in_beat_off_idx, beat_step in enumerate(frange(measure_step, measure_step + bar_resol, in_beat_tick_resol)):
            ...
            for instrument_idx in instrument_list:
              instrument_name = f"Instrument_{int(instrument_idx)}"  # Create an instrument event label
              t_notes = song_data['notes'][instrument_idx].get(beat_step)

              # If notes are present, process them
              if t_notes != None:
                # Add chord and tempo events if they haven't been added yet for this beat
                if not chord_tempo_flag:
                  if chord_text == None:
                    chord_text = 'Chord_N_N'  # Placeholder for missing chord
                  if tempo_text == None:
                    tempo_text = 'Tempo_N_N'  # Placeholder for missing tempo
                ...
                for note_dict in instrument_note_dict[instrument_idx]:
                    max_duration = max(instrument_note_dict[instrument_idx][note_dict], key=lambda x: x[0])
                    note_pitch_text = 'Note_Pitch_' + str(note_dict)
          ...
          for in_beat_off_idx, beat_step in enumerate(frange(measure_step, measure_step + bar_resol, in_beat_tick_resol)):
            ...
            for instrument_idx in instrument_list:
              ...
              if t_notes != None:
                ...
                # For each pitch, select the note with the longest duration and create note events
                for note_pitch in instrument_note_dict[instrument_idx]:
                    max_duration = max(instrument_note_dict[instrument_idx][note_pitch], key=lambda x: x[0])
                    note_pitch_text = 'Note_Pitch_' + str(note_pitch)
                    note_duration_text = 'Note_Duration_' + str(max_duration[0])
                    note_velocity_text = 'Note_Velocity_' + str(max_duration[1])
